import { Channel } from "@/lib/contact-data";
import { Section } from "@/app/components/Section";
import type { ContactInfo as SiteContactInfo, PlatformLinks } from "@/app/lib/settings-data";

function ChannelIcon({ channel }: { channel: Channel }) {
  if (channel.key === "whatsapp") {
    return (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="#25d366">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
      </svg>
    );
  }

  if (channel.key === "email") {
    return (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" stroke="#4f46e5" strokeWidth="2" strokeLinejoin="round" />
        <path d="M22 6l-10 7L2 6" stroke="#4f46e5" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }

  return (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
      <circle cx="12" cy="12" r="10" stroke="#f97316" strokeWidth="2" />
      <path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3" stroke="#f97316" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M12 17h.01" stroke="#f97316" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}

interface ChannelsGridProps {
  title: string;
  subtitle: string;
  channels: Channel[];
  siteContactInfo?: SiteContactInfo;
  sitePlatformLinks?: PlatformLinks;
}

export function ChannelsGrid({ title, subtitle, channels, siteContactInfo, sitePlatformLinks }: ChannelsGridProps) {
  return (
    <Section className="bg-white">
      <div className="max-w-[1100px] mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-[clamp(24px,3.5vw,36px)] font-bold text-text mb-2">
            {title}
          </h2>
          <p className="text-[16px] text-muted">{subtitle}</p>
        </div>

        <div className="channels-grid-3">
          {channels.filter(c => c.enabled).map((channel) => {
            const channelHref = channel.key === "whatsapp" && siteContactInfo
              ? `https://wa.me/${siteContactInfo.whatsappNumber}`
              : channel.key === "help" && sitePlatformLinks
                ? sitePlatformLinks.supportCenterUrl
                : channel.href;

            const channelValue = channel.key === "whatsapp" && siteContactInfo
              ? `+${siteContactInfo.whatsappNumber}`
              : channel.key === "help" && sitePlatformLinks?.supportCenterUrl
                ? sitePlatformLinks.supportCenterUrl.replace("https://", "").replace(/\/$/, "")
                : channel.displayText;

            return (
              <a
                key={channel.key}
                href={channelHref}
                target="_blank"
                rel="noopener noreferrer"
                className="channel-card"
              >
                <div className="channel-icon-box" style={{ background: channel.iconBg || "#F4F7FA" }}>
                  <ChannelIcon channel={channel} />
                </div>
                <div className="channel-name">{channel.label}</div>
                <div className="channel-val">{channelValue}</div>
                <span className="channel-action">
                  {channel.actionLabel || "تواصل الآن"}
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="mr-1">
                    <path d="M5 12h14M12 5l7 7-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </span>
              </a>
            );
          })}
        </div>
      </div>
    </Section>
  );
}
