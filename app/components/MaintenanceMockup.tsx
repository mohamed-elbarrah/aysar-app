    ...
    >
      ...
      >
        ...
        <div className="bg-navy px-2 sm:px-3 lg:px-4 py-2 sm:py-2.5 flex items-center justify-between">
          <span className="text-[9px] sm:text-[10px] lg:text-xs text-white/70 font-semibold">
            طلبات الصيانة
          </span>
        ...
        <div style={{ direction: "rtl" }}>
          ...
          <div className="grid grid-cols-[25px_1fr_60px_50px_45px_35px] sm:grid-cols-[30px_1fr_80px_60px_70px_45px] lg:grid-cols-[35px_1fr_100px_80px_90px_50px] gap-1 sm:gap-1.5 lg:gap-2 px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 bg-[#f9fafb] border-b border-[#f0f0f0]">
            {tableHeaders.map((header, idx) => (
              <span
                key={idx}
                className="text-[7px] sm:text-[8px] lg:text-xs font-bold text-[#718096]"
              >
                {header}
              </span>
            ))}
          ...
          {tableRows.map((row, idx) => (
            ...
            >
              <span className="text-[7px] sm:text-[9px] lg:text-xs text-[#718096]">
                {row.id}
              </span>
              <span className="text-[7px] sm:text-[9px] lg:text-sm font-semibold text-navy truncate">
                {row.name}
              </span>
              <span className="text-[6px] sm:text-[8px] lg:text-xs text-[#718096] truncate">
                {row.project}
              </span>
              <span className="text-[6px] sm:text-[8px] lg:text-xs text-[#718096]">
                {row.issue}
              </span>
              <span
