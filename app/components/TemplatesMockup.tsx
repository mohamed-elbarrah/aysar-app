  ...
  >
    ...
    >
      ...
      <div style={{ direction: "rtl" }}>
        {(() => {
          const gridCols =
            "grid grid-cols-[24px_4fr_1.2fr_1.6fr_1fr_1fr] sm:grid-cols-[28px_4fr_1.2fr_1.8fr_1fr_1fr] lg:grid-cols-[32px_5fr_1.2fr_2fr_1fr_1fr]";

          return (
            <>
              {/* Header */}
              <motion.div
                className={`${gridCols} gap-1 px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 bg-[#f9fafb] border-b border-[#f0f0f0]`}
                initial={{ opacity: 0 }}
              ...
              >
                {tableHeaders.map((header, idx) => (
                  <span
                    key={idx}
                    className="text-[8px] sm:text-[10px] lg:text-xs font-bold text-[#718096]"
                  >
                    {header}
                  </span>
                ))}
              ...
              {tableRows.map((row, idx) => (
                <motion.div
                  key={idx}
                  className={`${gridCols} gap-1 px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 items-center border-b border-[#f8f8f8] ${
                    row.altBg ? "bg-[#fafafa]" : ""
                  }`}
                ...
                >
                  <span className="text-[7px] sm:text-[9px] lg:text-xs text-[#718096] font-semibold">
                    {row.id}
                  </span>

                  <span className={`text-[8px] sm:text-[10px] lg:text-sm font-bold truncate ${
                    row.muted ? "text-[#b0b8c8]" : "text-navy"
                  }`}>
                    {row.name}
                  </span>
                  ...
                  {!row.muted ? (
                    <>
                      ...
                      {/* ✅ FIXED: Progress ABOVE percentage */}
                      <div className="flex justify-center items-center gap-1 ">
                        <div className="w-full h-0.5 sm:h-1 lg:h-1.5 bg-[#e0e6f0] rounded-full overflow-hidden">
                        ...
                        <span
                          className="text-[6px] sm:text-[8px] lg:text-xs font-bold text-center"
                    ...
                    <>
                      ...
                      <div className="flex flex-col gap-1">
                        <div className="h-0.5 sm:h-1 lg:h-1.5 bg-[#f0f0f0] rounded-full" />
                        <span className="text-[6px] sm:text-[8px] lg:text-xs text-center text-[#b0b8c8]">
                          —
                        </span>
                      </div>
